...
from resume.models import Profile, Language, EducationInformation, WorkExperience, ProfileDescription, ContactData, PersonalData
from resume.serializers import EducationInformationSerializer, WorkExperienceSerializer, ProfileSerializer
...
class ViewTests(TestCase):
    ...
    def assertNotEmpty(self, data):
        self.assertTrue(len(data) > 0, msg='Expected non-empty data')
    ...
    def test_education_information_viewset(self):
        ...
        get_url = reverse('educationinformation-detail',
                          args=[education_info.id])
        get_response = client.get(get_url)
        ...
        update_url = reverse('educationinformation-detail',
                             args=[education_info.id])
        update_response = client.put(
            update_url, update_data, format='json')

        self.assertEqual(update_response.status_code, status.HTTP_200_OK)
        self.assertEqual(EducationInformation.objects.get(
        ).degree, 'Bachelor of Engineering')
        self.assertEqual(
            EducationInformation.objects.get().institution, 'University D')

        # Delete education information data: It should return HTTP_204_NO_CONTENT
        delete_url = reverse('educationinformation-detail',
                             args=[education_info.id])
        delete_response = client.delete(delete_url)
    ...
    def test_work_experience_viewset(self):
        ...
        update_url = reverse('workexperience-detail',
                             args=[work_experience.id])
        update_response = client.put(update_url, update_data, format='json')
        ...
        delete_url = reverse('workexperience-detail',
                             args=[work_experience.id])
        delete_response = client.delete(delete_url)
    ...
    def test_profile_description_viewset(self):
        ...
        self.assertEqual(
            ProfileDescription.objects.get().summary, 'Test summary')
        ...
        self.assertEqual(
            ProfileDescription.objects.get().summary, 'Updated summary')

        # Delete contact profile description: It should return HTTP_204_NO_CONTENT
        delete_url = reverse('profiledescription-detail',
                             args=[description.id])
        delete_response = client.delete(delete_url)
        ...
        self.assertEqual(non_existent_response.status_code,
    ...
    def test_contact_data_viewset(self):
        ...
        self.assertEqual(ContactData.objects.get().website,
                         'https://example.com')
